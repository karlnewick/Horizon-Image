...
Start-Process "setup64.exe" -ArgumentList '/s /v" /qb REBOOT=R ADDLOCAL=ALL REMOVE=Hgfs,SVGA,VSS"' -Wait
...
if (-not$Running) {
  ...
  #Start-Process "setup64.exe" -ArgumentList '/s /v" /qb  REBOOT=R ADDLOCAL=ALL REMOVE=Hgfs,SVGA,VSS,AppDefense,NetworkIntrospection"' -Wait
  Start-Process "setup64.exe" -ArgumentList '/s /v" /qb  REBOOT=R ADDLOCAL=ALL REMOVE=Hgfs,SVGA,VSS"' -Wait
